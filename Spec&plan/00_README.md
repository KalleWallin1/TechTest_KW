...
| `06_State4_Design.md` | State 4 — square + pulsing breathing interpretation (square chosen over star for budget) |
...
- **State 4:** Open-ended in the brief — interpreted as a **square** with a pulsing breathing animation on stroke width and color intensity. Square was chosen over a 5-pointed star to keep implementation complexity in budget (a square is just a regular polygon at N=4, reusing the same morph machinery as triangle/hexagon — no special star SDF or vertex topology needed). CSV analysis (spec 09) revealed state 4 is the **terminal / leaving-scene state** for every object that has it, so an alpha fade-out is added to make this semantic visible.
